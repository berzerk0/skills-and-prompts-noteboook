#!/usr/bin/env python3
"""
Generate Claude Code agent wrappers from canonical YAML definitions.

Usage:
    python meta/generate_claude.py [--all | --skill SKILL_NAME]
    
Examples:
    # Generate all skills
    python meta/generate_claude.py --all
    
    # Generate a specific skill
    python meta/generate_claude.py --skill timestamp
"""
...
# Tool names for Claude Code
CLAUDE_TOOLS = ["Read", "Write", "Edit", "Bash", "Grep", "Glob", "ListDirectory"]
...
def generate_skill_md(skill_name: str, data: dict) -> str:
    """Generate portable SKILL.md file for Claude Code."""
    name = data.get('name', skill_name)
    description = data.get('description', '')
...
def generate_skill(skill_name: str) -> None:
    """Generate all Claude Code files for a skill."""
    print(f"\nGenerating Claude Code files for: {skill_name}")
    ...
    agent_path = CLAUDE_AGENTS_DIR / f"{skill_name}.md"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate Claude Code agent wrappers from canonical YAML'
    )
    parser.add_argument(
        '--all',
        action='store_true',
    ...
    if args.skill:
        ...
        print("\n✓ All Claude Code files generated")
    else:
        parser.print_help()
