...
description: Writing documents for agents. Use when creating or editing skills, or modifying AGENTS.md, VIBE.md, or CLAUDE.md.
license: MIT
compatibility: [claude, pi, vibe]
---

Reference for writing any document an agent consumes: a skill, an `AGENTS.md` / `VIBE.md` / `CLAUDE.md`, a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable, since the agent takes the same _process_ every run rather than producing the same output.
